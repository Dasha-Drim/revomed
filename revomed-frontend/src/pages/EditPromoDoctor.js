...
// styles
import './EditPromoDoctor.scss';


let EditPromoDoctor = (props) => {
	...
	useEffect(() => {
		...
		// get clinic catalog
		let getPromo = async () => {
			let res = await API.get('/doctors/promos/'+props.match.params.promoId);
	...
	let postPromo = async (data) => {
		let res = await API.post('/doctors/promos', data);
		return await res.data;
	}
	let putPromo = async (data) => {
		let res = await API.put('/doctors/promos', data);
		return await res.data;
	}

	const postPromoForm = useRef(null);
	let postPromoRequest = (e) => {
			console.log("вукекевоч ооооооооооооочт  1234")
			e.preventDefault();
...
export default EditPromoDoctor;