...
import InputPlusChecking from "../elements/InputPlusChecking/InputPlusChecking";
...
let EditBanner = (props) => {
	let [postInfo, setPostInfo] = useState(null);
	let [status, setStatus] = useState(null)
	let [message, setMessage] = useState("")
	//let [shops, setShops] = useState([{ shop: "", link: ""}]);
	...
	useEffect(() => {
		...
		let getPost = async (id) => {
			let res = await API.get("/farms/banners/"+id);
			return await res.data;
		}
		

		getPost(props.match.params.idBanner).then((result) => {
			setPostInfo(result.banner);
			setStatus(result.banner.status);
			setMessage(result.banner.message.text);
			setUploadedPhotoProfileStyles({backgroundImage: "url("+result.banner.photo+")", backgroundPosition: "center", backgroundSize: "cover"})
		})
	...
	let editProfileFormHandler = (e) => {
		...
		postData.append("idBanner", props.match.params.idBanner);
						...
						<form onSubmit={editProfileFormHandler} ref={editProfileForm} className="col-12 py-5">
							<div className="row">
								...
								<div className="col-12 col-md-6 mb-4">
									<Input name="name" label="Название (не публично)" value={postInfo.name} validation="notEmpty" errors={errors} />
								</div>
								<div className="col-12 col-md-6 mb-4">
									<Input name="link" label="Ссылка" value={postInfo.link} validation="notEmpty" errors={errors} />
								</div>
