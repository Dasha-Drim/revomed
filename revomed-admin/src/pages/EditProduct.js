...
let EditProduct = (props) => {
	...
	useEffect(() => {
		...
		let getPost = async (id) => {
			let res = await API.get("/farms/products/"+id);
			return await res.data;
		}
		

		getPost(props.match.params.idProduct).then((result) => {
			setPostInfo(result.product);
			setStatus(result.product.status);
			setMessage(result.product.message.text)
			setShops(result.product.shops);
			setUploadedPhotoProfileStyles({backgroundImage: "url("+result.product.photo+")", backgroundPosition: "center", backgroundSize: "cover"})
		})
	...
	let editProfileFormHandler = (e) => {
		...
		let postData = new FormData(editProfileForm.current);
			...
			postData.append("idProduct", props.match.params.idProduct);
						...
						<form onSubmit={editProfileFormHandler} ref={editProfileForm} className="col-12 py-5">
							<div className="row">
								...
								<div className="col-12 mb-4 mt-3">
									<Textarea name="description" value={postInfo.description} placeholder="Описание продукта" readOnly/>									
								</div>
								<div className="col-12 mb-4">
									<InputPlusChecking name="shops" value={shops} errors={errors} label="Магазины" validation="notEmpty"/>
								</div>
